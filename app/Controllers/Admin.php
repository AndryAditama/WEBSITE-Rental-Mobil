...
class Admin extends BaseController
{
   ...
   public function simpan_mobil()
   {
      if (!$this->validate([
         ...
         'no_pol' => [
            'rules' => 'required|is_unique[data_mobil.no_pol]',
            'errors' => [
               'required' => 'no pol harus di isi',
               'is_unique' => 'no pol sudah ada'
            ]
         ],
         'biaya' => [
            'rules' => 'required',
   ...
   public function hapus_mobil($id_mobil)
   {
      ...
      if (isset($get_id['J_mobilID']) == $id_mobil) {
         // dd('true');
   ...
   public function hapus_penyewa($id_penyewa)
   {
      $builder = $this->db->table('jadwal_sewa');
      $get_Idpenyewa = $builder->get()->getRowArray();

      if (isset($get_Idpenyewa['J_penyewaID']) == $id_penyewa) {
         session()->setFlashdata('pesan_merah', 'data gagal di hapus karena id ' . $id_penyewa . ' ada data jadwal atau transaksi yang sedang berjalan');
         return redirect()->to(site_url('admin/data_penyewa'));
      }
   ...
   public function buat_jadwal()
   {
      ...
      if ($this->request->getVar('tanggal_kembali') < $this->request->getVar('tanggal_sewa')) {
         session()->setFlashdata('pesan_merah', 'tanggal kembali tidak boleh kecil dari tanggal sewa');
   ...
   public function refresh_jadwal()
   {
      $timeNow = date('H:i:s');
      $builder = $this->db->table('jadwal_sewa');
      $cek_data = $builder->get();
      $jml = 1;
      foreach ($cek_data->getResult() as $row) {
         $penyewa_id = $row->J_penyewaID;

         if ($timeNow > $row->tutup_boking) {
            // return redirect()->to(site_url('admin/data_jadwal'));
            $builder = $this->db->table('jadwal_sewa');
            $builder->set('status', 'mulai');
            $builder->where('J_penyewaID', $penyewa_id);
      ...
      if (isset($get_jml)) {
         session()->setFlashdata('pesan_hijau', 'Berhasil ditambahkan ke data transaksi sebanyak ' . $get_jml . ' data');
      } else {
         session()->setFlashdata('pesan_hijau', 'Tidak ada data yang harus di refresh');
      }
      return redirect()->to(site_url('admin/data_jadwal'));
   }
   ...
   public function refresh_detail()
   {
      $now = Time::now();
      $builder = $this->db->table('transaksi');
      $cek_data = $builder->get();
      ...
      if (isset($jml_terlambat)) {
         session()->setFlashdata('pesan_merah', 'Data penyewa yang terlambat sebanyak ' . $jml_terlambat . ' data');
      } else {
         session()->setFlashdata('pesan_hijau', 'Data diupdate sebanyak ' . $jml_update . ' data');
      }
   ...
   public function transaksi_selesai()
   {
      ...
      $builder->select('selesai_sewa.id_selesai, selesai_sewa.boking_waktu, selesai_sewa.sewa, selesai_sewa.kembali, selesai_sewa.biaya_sewaMobil, selesai_sewa.total_hariSewa, selesai_sewa.total_keseluruhan, selesai_sewa.total_biayaSewa, data_penyewa.nama, data_mobil.merek');
      $builder->join('data_penyewa', 'selesai_sewa.S_penyewaID = data_penyewa.id_penyewa');
      $builder->join('data_mobil', 'selesai_sewa.S_mobilID = data_mobil.id_mobil')->orderBy('sewa', 'DESC');

      $bulan = $this->request->getGet('bulan');
      $tahun = $this->request->getGet('tahun');

      // Tambahkan kondisi berdasarkan input filter
      if (!empty($bulan)) {
         $filter = date('Y' . '-' . $bulan);
         $builder->like('sewa', $filter, 'after');
      }
      if (!empty($tahun)) {
         $builder->where('YEAR(selesai_sewa.sewa)', $tahun);
         $filter = $tahun;
      }
      if (!empty($bulan) && !empty($tahun)) {
         $builder->where('MONTH(selesai_sewa.sewa)', $bulan);
         $builder->where('YEAR(selesai_sewa.sewa)', $tahun);
         $filter = $tahun . '-' . $bulan;
      }
      if (empty($bulan) && empty($tahun)) {
      ...
      // dd($filter);
      $query = $builder->get();
      $totalData = $query->getResult();

      $queryTotal = $builder->selectSum('total_biayaSewa')->get();
      $resultTotal = $queryTotal->getRow();

      $tanggalHariIni = date('Y-m-d');
      $bulanIni = date('Y-m');
      $tahunIni = date('Y');

      $builder->selectSum('total_biayaSewa');
      $builder->like('sewa', $tanggalHariIni, 'after'); // Menggunakan 'after' agar formatnya seperti 'YYYY-MM-DD%'
      $queryHariIni = $builder->get();
      $resultHariIni = $queryHariIni->getRow();
      // dd($resultHariIni);

      $builder->selectSum('total_biayaSewa');
      $builder->like('sewa', $bulanIni, 'after'); // Menggunakan 'after' agar formatnya seperti 'YYYY-MM-DD%'
      $queryBulanIni = $builder->get();
      $resultBulanIni = $queryBulanIni->getRow();

      $builder->selectSum('total_biayaSewa');
      $builder->like('sewa', $tahunIni, 'after'); // Menggunakan 'after' agar formatnya seperti 'YYYY-MM-DD%'
      $queryTahunIni = $builder->get();
      $resultTahunIni = $queryTahunIni->getRow();

      // Menghitung total pendapatan berdasarkan filter
      $builderClone = clone $builder;
      $builderClone->selectSum('selesai_sewa.total_biayaSewa', 'totalPendapatan');
      $builderClone->like('sewa', $filter, 'after');
      $hitung = $builderClone->get();
      $totalPendapatan = $hitung->getRow()->totalPendapatan;

      $data = [
         'tittle' => 'Riwayat Transaksi',
         'total' => $resultTotal->total_biayaSewa,
         'harian' => $resultHariIni->total_biayaSewa,
         'bulanan' => $resultBulanIni->total_biayaSewa,
         'tahunan' => $resultTahunIni->total_biayaSewa,
         'selesai' => $totalData,
